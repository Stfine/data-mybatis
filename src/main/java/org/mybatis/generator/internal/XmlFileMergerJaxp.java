...
public class XmlFileMergerJaxp {
    ...
    public static String getMergedSource(GeneratedXmlFile generatedXmlFile,
        ...
        try {
            ...
            Map<String, Node> existingChildrenMap=new HashMap<String, Node>();
            for (int i = 0; i < existingRootElement.getChildNodes().getLength(); i++) {
            	 Node node = existingRootElement.getChildNodes().item(i);
            	 
                 if(node.getAttributes() != null){
                 	String id = node.getAttributes().getNamedItem("id").getNodeValue();
                 	String name = node.getNodeName();
                 	existingChildrenMap.put(name + ":" +id , node);
                 }
			}
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (firstChild == null) {
                    ...
                    if(node.getAttributes() != null){
                    	...
                    	if(id.equals("BOResultMap")){
                    		continue;
                    	}
                    	
                    	Node existingNode = existingChildrenMap.get(name + ":" +id );
                    	if(existingNode != null){
                    		existingNode.getParentNode().replaceChild(newNode, existingNode);
                    	}
                    }else{
